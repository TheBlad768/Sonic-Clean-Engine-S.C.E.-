; ---------------------------------------------------------------------------
; Hidden Monitor (Object)
; ---------------------------------------------------------------------------

; dynamic object variables

; =============== S U B R O U T I N E =======================================

Obj_HiddenMonitor:

		; init
		lea	ObjDat_HiddenMonitor(pc),a1
		jsr	(SetUp_ObjAttributes).w
		move.l	#.main,code_addr(a0)
		move.w	#bytes_to_word(30/2,30/2),y_radius(a0)				; set y_radius and x_radius
		move.b	#collision_type.npc.monitor,collision_type(a0)			; set monitor collision type
		move.w	#bytes_to_word(32/2,32/2),collision_height(a0)			; set height and width collision
		move.b	subtype.byte(a0),anim(a0)					; set monitor content id
		rts
; ---------------------------------------------------------------------------

.main
		move.w	(Signpost_addr).w,d0						; address is empty?
		beq.s	.notdraw							; if it is, branch
		movea.w	d0,a1								; get signpost address
		btst	#0,state_flags(a1)
		beq.s	.notdraw							; if signpost hasn't landed, branch

		; check xy positions
		lea	HiddenMonitor_Range(pc),a2
		jsr	(Check_InTheirRange).w
		bne.s	.bounceup

		; landed
		sfx	sfx_GroundSlide							; if signpost has landed
		move.l	#Delete_Sprite_If_Not_In_Range,code_addr(a0)			; not draw hidden monitor

.notdraw
		jmp	(Delete_Sprite_If_Not_In_Range).w
; ---------------------------------------------------------------------------

.bounceup
		bclr	#0,state_flags(a1)						; if signpost has landed and is in range
		move.l	#Obj_Monitor.main,code_addr(a0)					; make this object a monitor
		st	routine_secondary(a0)						; set the monitor's routine_secondary counter
		move.w	#-$500,y_vel(a0)
		sfx	sfx_BubbleAttack						; play sfx

		; check
		bclr	#render_flags.x_flip,render_flags(a0)
		beq.s	.draw
		bset	#high_priority_bit,art_tile(a0)					; high priority
		clr.b	status(a0)

.draw
		jmp	(Sprite_OnScreen_Test).w

; =============== S U B R O U T I N E =======================================

; init
ObjDat_HiddenMonitor:	subObjData Map_Monitor, ArtTile_Monitors, 0, FALSE, 32, 28, 5, 0, collision_type.npc.none, 0, 0

HiddenMonitor_Range:
		dc.w -14, 28	; xpos
		dc.w -128, 192	; ypos
