...
Run_PalRotationScript:
		...
		; check disable flag
		tst.b	(Palette_rotation_disable).w					; is flag set?
...
.wait
		...
		beq.s	.return								; if 0, return
		subq.b	#1,2(a1)							; decrement delay
		bpl.s	.next								; if still positive, go to next entry
...
.next
		...
		bra.s	.wait								; run the code again
; ---------------------------------------------------------------------------

.main
		move.b	3(a2),d2							; load additional parameter to d2
		beq.s	.restart							; FE (restart)
		neg.w	d1
		jmp	.index-4(pc,d1.w)
; ---------------------------------------------------------------------------

.index
		bra.s	.change								; FC (loop)
; ---------------------------------------------------------------------------

		; custom code								; FA (run)
		addq.b	#1,3(a1)
		cmp.b	3(a1),d2
		bhi.s	.restart							; wait for counter to finish
